...
resource "azurerm_subnet" "cluster_jumpnet"{     
  name                 = "jumpnet"
...
resource "azurerm_network_interface" "cluster_jumpbox_nic" {
  ...
  ip_configuration {
    name                          = "internal"
    subnet_id                     = azurerm_subnet.cluster_jumpnet.id
    private_ip_address_allocation = "Dynamic"
  }
}

resource "azurerm_linux_virtual_machine" "cluster_jumpbox" {
  ...
  network_interface_ids = [
    azurerm_network_interface.cluster_jumpbox_nic.id,
  ]
